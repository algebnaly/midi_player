...
fn handle_right_click(widget: &super::DrumRollWidget, x: f64, y: f64) {
    ...
    if let Some(target_pitch) = target_pitch {
        ...
        if let Some(midi) = &mut *imp.data.borrow_mut() {
            if let Some(hit) = hit_test_note(midi, act_track, zx, zy, abs_x, target_pitch) {
...
fn handle_scroll(
    ...
    let zy = *imp.zoom_y.borrow();
    let mut total_rows = 128.0;
    if let Some(midi) = &*imp.data.borrow() {
        if let Some(track) = midi.tracks.get(*imp.active_track.borrow()) {
            if let crate::midi::TrackMode::Drum(dm) = &track.mode {
                total_rows = dm.row_count() as f64;
            }
        }
    }
    let max_scroll_y = total_rows * zy - widget.height() as f64;
