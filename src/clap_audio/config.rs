//! Audio configuration types for CLAP plugin hosting.
//!
//! Contains [`FullAudioConfig`] (the master config shared between CPAL and the
//! CLAP plugin), [`PluginAudioPortsConfig`] (port layout info), and helpers for
//! device capability negotiation.
//!
//! Several functions are currently unused because we construct a fixed stereo
//! config in [`ClapPluginWrapper`](crate::clap_host::ClapPluginWrapper).
//! They will become active when dynamic port negotiation is implemented.
#![allow(dead_code)]

use crate::clap_host::MidiPlayerHost;
...
impl FullAudioConfig {
    ...
    pub fn find_best_from(
        device: &Device,
        instance: &mut PluginInstance<MidiPlayerHost>,
