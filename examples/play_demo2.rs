...
fn main() {
    ...
    let mut sf2_bass = File::open("FlameStudios/FS_Ibanez_Electric_Bass_Guitar/FS Ibanez Electric Bass Guitar.sf2").unwrap();
    ...
    synth_bass.send_event(MidiEvent::ProgramChange { channel: 0, program_id: 0 }).unwrap();
    synth_guitar.send_event(MidiEvent::ProgramChange { channel: 1, program_id: 0 }).unwrap();
        ...
        cpal::SampleFormat::F32 => device.build_output_stream(
            ...
            move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
                ...
                if let Ok(mut s) = d_clone.try_lock() {
                    tmp_l.fill(0.0); tmp_r.fill(0.0);
                    s.write((&mut tmp_l[..], &mut tmp_r[..]));
                    for i in 0..frames { buf_l[i] += tmp_l[i]; buf_r[i] += tmp_r[i]; }
                }
                // Render Bass
    ...
    macro_rules! drum {
        ($key:expr, $vel:expr) => { d.send_event(MidiEvent::NoteOn { channel: 9, key: $key, vel: $vel }).unwrap(); }
    }
    macro_rules! bass {
        ($key:expr, $vel:expr) => { b.send_event(MidiEvent::NoteOn { channel: 0, key: $key, vel: $vel }).unwrap(); }
    }
    macro_rules! guitar {
        ($key:expr, $vel:expr) => { g.send_event(MidiEvent::NoteOn { channel: 1, key: $key, vel: $vel }).unwrap(); }
    }
    ...
    for bar in 0..4 {
        ...
        drum!(42, 90);  // Hihat
        bass!(36, 110); // C2
        guitar!(60, 100); guitar!(64, 90); guitar!(67, 90); // C major chord
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        ...
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        b.send_event(MidiEvent::NoteOff { channel: 0, key: 48 }).unwrap();

        // Beat 2 (Snare + Hihat)
        drum!(38, 110); // Snare
        drum!(42, 90);
        bass!(36, 100); // C2
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 60 }).unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 64 }).unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 67 }).unwrap();
        // Guitar chug (muted-ish feel by short duration)
        guitar!(60, 100); guitar!(67, 100); // C power chord
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 60 }).unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 67 }).unwrap();
        ...
        drum!(42, 90);
        bass!(43, 110); // G2
        guitar!(67, 100); guitar!(71, 90); guitar!(74, 90); // G major chord
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        ...
        bass!(36, 100);
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 67 }).unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 71 }).unwrap();
        g.send_event(MidiEvent::NoteOff { channel: 1, key: 74 }).unwrap();
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        ...
        drop(d); drop(b); drop(g); sleep(eighth);
        d = drums_arc.lock().unwrap(); b = bass_arc.lock().unwrap(); g = guitar_arc.lock().unwrap();
        b.send_event(MidiEvent::NoteOff { channel: 0, key: 36 }).unwrap();
        b.send_event(MidiEvent::NoteOff { channel: 0, key: 43 }).unwrap();
    }
    
    println!("Demo finished!");
    drop(d); drop(b); drop(g);
