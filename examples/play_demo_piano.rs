...
fn main() {
    ...
    let mut sf2 = File::open("Pianos/SalamanderGrandPiano-SF2-V3+20200602/SalamanderGrandPiano-V3+20200602.sf2").unwrap();
    ...
    macro_rules! play_chord {
        ($notes:expr, $vel:expr, $dur:expr) => {
            for &note in $notes.iter() {
                s.send_event(MidiEvent::NoteOn { channel: 0, key: note, vel: $vel }).unwrap();
            }
            drop(s);
            sleep(Duration::from_millis($dur));
            s = synth_arc.lock().unwrap();
            for &note in $notes.iter() {
                s.send_event(MidiEvent::NoteOff { channel: 0, key: note }).unwrap();
            }
        }
    }
